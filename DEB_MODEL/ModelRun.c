...
double MODEL_PARAMETERISATION(double X[NoParameters] ,
...
for( int t = 0; t < NoDays; t++)
{
  ...
  for( int i = 0; i < IndNum; i++)
  {
    ...
    // MODEL FIT ERROR FOR ABUNDANCE
    for( int i = 0; i < NumAbundancePoints ; i++){
        *result = *result + 1*((ABUNDANCE_FIT[i] -   log10(ModelledAbundance[i]+1) )/ ABUNDANCE_FIT[i])*((ABUNDANCE_FIT[i] -   log10(ModelledAbundance[i]+1) )/ ABUNDANCE_FIT[i]) ;

    
    }

    
    // MODEL FIT ERROR FOR WEIGHT, LENGTH, RS
    for( int i = 0; i < NumMeanLengths ; i++){
        
             *result = *result +
            1*((MEAN_WL_RS[i] -  ModelledReserveRatio[i]/ModelledDenominator[i])/MEAN_WL_RS[i]  )*   (  (MEAN_WL_RS[i] -  ModelledReserveRatio[i]/ModelledDenominator[i])/MEAN_WL_RS[i] ) +
            1*((MEAN_WL_LENGTH[i] -  ModelledLength[i]/ModelledDenominator[i])/MEAN_WL_LENGTH[i]  )*   (  (MEAN_WL_LENGTH[i] -  ModelledLength[i]/ModelledDenominator[i])/MEAN_WL_LENGTH[i] ) +
            1*( (MEAN_WL_WEIGHT[i] -  ModelledWeight[i]/ModelledDenominator[i])/MEAN_WL_WEIGHT[i]  ) *( (MEAN_WL_WEIGHT[i] -  ModelledWeight[i]/ModelledDenominator[i])/MEAN_WL_WEIGHT[i]  ) ;
    ...
    static const char filename98[] = "ModLengthWeighttest.txt";
    FILE *file98 = fopen ( filename98, "w" );
    for( int i = 0; i < NumMeanLengths ; i++)
    {
        fprintf(file98, "%G %G %G %G %G %G\n",MEAN_WL_LENGTH[i],ModelledLength[i]/ModelledDenominator[i] ,  MEAN_WL_WEIGHT[i] , ModelledWeight[i]/ModelledDenominator[i],MEAN_WL_RS[i], ModelledReserveRatio[i]/ModelledDenominator[i]) ;
    ...
    /*
    static const char filename99[] = "ModAbundancetest.txt";
    FILE *file99 = fopen ( filename99, "w" );
    for( int i = 0; i < NumAbundancePoints ; i++)
    {
        fprintf(file99, "%G %G\n",ABUNDANCE_FIT[i],ModelledAbundance[i]) ;
 ...
 */
    ...
    if(*result != *result){*result = 1000000000;} 
